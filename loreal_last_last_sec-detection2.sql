...
create schema nstage
CREATE TABLE [nstage].[tbl_testfor_TrackOne](
	...
	CONSTRAINT [PK_stage_tbl_testfor_TrackOne] PRIMARY KEY CLUSTERED 
		(
			[email] ASC
		)
)
...
alter table  ndev.[tbl_testfor_TrackOne] set (SYSTEM_VERSIONING = OFF)
drop table [ndev].[tbl_testfor_TrackOne]
go

CREATE TABLE [ndev].[tbl_testfor_TrackOne]
(
	[email]				[nvarchar](128) not null,
	[title]				[nvarchar](510) NULL,
	[first_name]		[nvarchar](510) NULL,
	...
	CONSTRAINT [PK_tbl_testfor_TrackOne] PRIMARY KEY CLUSTERED 
	(
		[email] ASC
...
WITH
	( SYSTEM_VERSIONING = ON ( HISTORY_TABLE = [ndev].[tbl_testfor_TrackOne_hist] ))
...
alter table  ndev.[tbl_C1_interface] set (SYSTEM_VERSIONING = OFF)
DROP TABLE ndev.[tbl_C1_interface]
GO
DROP TABLE ndev.[tbl_C1_interface_hist]
go
CREATE TABLE [ndev].[tbl_C1_interface] ([NID] [UNIQUEIDENTIFIER] NULL,
                                        ...
                                        CONSTRAINT [PK_tbl_C1_interface]
                                            PRIMARY KEY CLUSTERED ([EMAIL] ASC)
                                            WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF,
                                                  ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY],
                                        PERIOD FOR SYSTEM_TIME([ValidFrom], [ValidTo])) ON [PRIMARY]
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [ndev].[tbl_C1_interface_hist]));
GO
...
truncate table [nstage].[tbl_testfor_TrackOne] 

insert into  [nstage].[tbl_testfor_TrackOne] 
...
exec ndev.load_testfor_TrackOne
--
-- updatepad
-- 1
truncate table [nstage].[tbl_testfor_TrackOne] 
...
select * from  [nstage].[tbl_testfor_TrackOne] 

--
-- v1
--

create procedure ndev.load_testfor_TrackOne
as
	begin

		MERGE INTO [ndev].[tbl_testfor_TrackOne]	AS t
			 USING [nstage].[tbl_testfor_TrackOne]	AS s 
				ON t.email = s.email
...
-- v3
--
ALTER PROCEDURE ndev.load_testfor_TrackOne
AS
BEGIN

    DELETE FROM ndev.tbl_C1_interface
     WHERE EXISTS (   SELECT 1
                        FROM [nstage].[tbl_testfor_TrackOne] s
                       WHERE s.email = ndev.tbl_C1_interface.EMAIL);

    --
    --	delta calculation and persisting against
    --	delta hive

    INSERT INTO ndev.tbl_C1_interface
    SELECT            NEWID(),
                      a.email,
      ...
      FROM            [nstage].[tbl_testfor_TrackOne] s
      LEFT OUTER JOIN [ndev].[tbl_testfor_TrackOne] a
        ON s.email = a.email;
    ...
    USING [nstage].[tbl_testfor_TrackOne] AS s
     ...
     WHEN NOT MATCHED BY TARGET THEN
        INSERT ([email],
                ...
                [zip_code],
                [city])
        VALUES ([email], [title], [first_name], [last_name], [street], [house_number], [zip_code], [city])
...
SELECT * FROM [ndev].tbl_C1_interface FOR SYSTEM_TIME all

select * from [ndev].[tbl_testfor_TrackOne_hist]

select  row_number() over (partition by [email] order by [ValidTo] desc) key_sequence_nr, *
		from [ndev].[tbl_testfor_TrackOne_hist]
...
select a.email
  ...
  from 
		[ndev].[tbl_testfor_TrackOne] a
		left outer join
		[ndev].[tbl_testfor_TrackOne_hist] h
		on		a.[email]	= h.[email]
...
--
select a.email
	 , coalesce(nullif(a.[first_name], '')	, coalesce(nullif(h.[first_name], '')+'*', (select top 1 [first_name] from [ndev].[tbl_testfor_TrackOne_hist] where [email] = a.[email] and nullif([first_name], '') is not null order by ValidTo desc))+'x')		[first_name]
	 , coalesce(nullif(a.[last_name], '')	, coalesce(nullif(h.[last_name], '')+'*', (select top 1 [last_name] from [ndev].[tbl_testfor_TrackOne_hist] where [email] = a.[email] and nullif([last_name], '') is not null order by ValidTo desc))+'x')		[last_name]
	 , coalesce(nullif(a.[street], '')	, coalesce(nullif(h.[street], '')+'*', (select top 1 [street] from [ndev].[tbl_testfor_TrackOne_hist] where [email] = a.[email] and nullif([street], '') is not null order by ValidTo desc))+'x')		[street]
	 , coalesce(nullif(a.[house_number], ''), coalesce(nullif(h.[house_number], '')+'*', (select top 1 [house_number] from [ndev].[tbl_testfor_TrackOne_hist] where [email] = a.[email] and  nullif([house_number], '') is not null order by ValidTo desc))+'x')		[house_number]
	 , coalesce(nullif(a.[zip_code], '')	, coalesce(nullif(h.[zip_code], '')+'*', (select top 1 [zip_code] from [ndev].[tbl_testfor_TrackOne_hist] where [email] = a.[email] and nullif([zip_code], '') is not null order by ValidTo desc))+'x')		[zip_code]
	 , coalesce(nullif(a.[city], '')		, coalesce(nullif(h.[city], '')+'*', (select top 1 [city] from [ndev].[tbl_testfor_TrackOne_hist] where [email] = a.[email] and  nullif([city], '') is not null order by ValidTo desc))+'x')		[city]
...
select a.email
  ...
  from 
	...
	select * FROM [ndev].[tbl_testfor_TrackOne_hist] FOR SYSTEM_TIME ALL
			where [email] = 'nadja@gmail.coom'
				order by [VALIDTO] desc

	select * FROM [ndev].[tbl_testfor_TrackOne] FOR SYSTEM_TIME ALL
...
DELETE FROM ndev.tbl_C1_interface
 WHERE ndev.tbl_C1_interface.EMAIL IN (   SELECT email
                                            FROM [ndev].[tbl_testfor_TrackOne] h
...
WITH histset
  AS (SELECT TOP 1 *
        FROM [ndev].[tbl_testfor_TrackOne_hist] h
       WHERE h.email = 'nadja@gmail.coom'
...
SELECT            NEWID(),
                  ...
                  ndev.fn_ahc(a.countryTRY, h.country) COUNTRYTRY
  FROM            masterdata h
  LEFT OUTER JOIN [ndev].tbl_C1_interface a
    ON a.email = h.email;
...
SELECT * FROM [ndev].[tbl_testfor_TrackOne] FOR SYSTEM_TIME ALL WHERE email = 'nadja@gmail.coom' ORDER BY ValidTo desc
SELECT * FROM  ndev.tbl_C1_interface FOR SYSTEM_TIME ALL WHERE email = 'nadja@gmail.coom' ORDER BY ValidTo desc
SELECT *FROM  ndev.tbl_C1_interface_hist c1ih
...
SELECT UPPER(REPLACE('ndev.fn_ahc(a.CCCC,h.CCCC) CCCC', 'CCCC', column_name))
	 + iif (ORDINAL_POSITION < (SELECT MAX(ORDINAL_POSITION) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA= 'ndev' AND TABLE_NAME = 'tbl_testfor_TrackOne' AND column_name NOT LIKE 'VALID%'), ',','')
  FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA= 'ndev' AND TABLE_NAME = 'tbl_testfor_TrackOne' AND column_name NOT LIKE 'VALID%' ORDER BY ORDINAL_POSITION;

--
--	CDOE:
--	Creating the column selector for column-based delta set determination
DECLARE @tableName    NVARCHAR(64)   = 'tbl_C1_interface',
...
SELECT            *
  FROM            [ndev].[tbl_testfor_TrackOne] a
  LEFT OUTER JOIN [ndev].[tbl_testfor_TrackOne_hist] h
...
dbcc checkconstraints([ndev].[tbl_testfor_TrackOne] )

select getdate()

select * from [ndev].[tbl_testfor_TrackOne] where email = 'andrea@gmail.coom'
select * from [ndev].[tbl_testfor_TrackOne_hist] where email = 'andrea@gmail.coom' order by validto desc

update [ndev].[tbl_testfor_TrackOne] set title= 'Mrs.' where email = 'andrea@gmail.coom'
