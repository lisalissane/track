...
SET @c = 'gma_customer' + '_' + replace(replace(Cast(GETDATE() AS NVARCHAR),' ','_'),':','_')

if exists (select * from sysobjects where name='gma_customer' and xtype='U')
Execute sp_RENAME '[gma_customer]', @c

GO

CREATE TABLE [gma_customer] ( 
...
,[voucherPositionisPOSRedirect,isTableDelivery,totalAmount,bdTotalAmount,totalTaxAmount,DiscountAmount,ItemForExtraction,productCode,description,dtName,Item_PromotionID,quantity,quantitySold,quantityPromo,unitPrice,adUnitPrice,totalPrice,adTotalPrice,totalTax,netPrice,itemLevel,itemCode,prodAction,isGrillLine,taxID,taxRate,voucherPosition] [NVARCHAR] (510) NULL
) ON [PRIMARY] 
GO
BEGIN TRANSACTION
BEGIN TRY
BULK INSERT [gma_customer]
From 
'\\trell\Freigaben\01_TABLEAU_REPORTS\MCD\01_DATA\01_Upload\Gma-customerID_000000000002.txt'
WITH 
(
    CODEPAGE = 'ACP',
    FIRSTROW = 2,
...
IF NOT EXISTS (SELECT * FROM sys.Columns WHERE object_id = OBJECT_ID(N'[dbo].[gma_customer]') AND name = 'SOURCE') 
ALTER TABLE [gma_customer] ADD [SOURCE] NVARCHAR(255)
GO

UPDATE [gma_customer] SET [SOURCE] = 'Gma-customerID_000000000002.txt'
GO

INSERT INTO Vodafone_Cleaner.dbo.tbl_protokoll(cdate, project, developer, description, code, Counts) 
SELECT getdate() as cdate 
, 'gma_customer' as project 
