...
(
	SELECT 1
	FROM INFORMATION_SCHEMA.TABLES
	WHERE UPPER(TABLE_NAME) = UPPER('tbl_GMA_Export_Promotions') AND 
		  UPPER(TABLE_SCHEMA) = UPPER('dbo')
)
	DROP TABLE dbo.tbl_GMA_Export_Promotions;
CREATE TABLE [dbo].[tbl_GMA_Export_Promotions](
	[tec_sid] [bigint] IDENTITY(1,1) NOT NULL,
	[tec_import_ts] [datetime2](6) NULL,
	[BusinessDay] [datetime2](7) NOT NULL,
	[PromotionID] [int] NOT NULL,
	[PromotionName] [nvarchar](100) NOT NULL,
	[PromotionMinBusinessDay] [datetime2](7) NOT NULL,
	[PromotionMaxBusinessDay] [datetime2](7) NOT NULL,
	[GuestCountTotal] [int] NOT NULL,
	[GuestCountFranchise] [int] NOT NULL,
	[GuestCountMcOpCo] [int] NOT NULL,
	[GrossAmount] [nvarchar](50) NOT NULL,
	[NetSalesAmountAfterDiscount] [nvarchar](50) NOT NULL,
	[UnitsSold] [int] NOT NULL,
	[AvgFPCostAvg] [nvarchar](50) NOT NULL,
	[NetSalesAmount] [nvarchar](50) NOT NULL,
	[FPCost] [nvarchar](50) NOT NULL,
	[StoreCountFranchise] [int] NOT NULL,
	[StoreCountMcOpCo] [int] NOT NULL,
	[StoreCountTotal] [int] NOT NULL
) ON [PRIMARY]

ALTER TABLE [dbo].[tbl_GMA_Export_Promotions] ADD  DEFAULT (sysdatetime()) FOR [tec_import_ts]
...
(
	...
	WHERE UPPER(TABLE_NAME) = UPPER('vw_ins_GMA_Export_Promotions') AND 
		  UPPER(TABLE_SCHEMA) = UPPER('dbo')
)
	BEGIN
		DROP VIEW dbo.vw_ins_GMA_Export_Promotions;
	END;
GO
create view [dbo].[vw_ins_GMA_Export_Promotions]
as
SELECT [BusinessDay]
      ,[PromotionID]
      ,[PromotionName]
...
as
SELECT  
	   tbl_GMA_Export_Promotions.[BusinessDay], 
       ...
       CAST(replace(tbl_GMA_Export_Promotions.[GrossAmount], ',', '.') AS FLOAT) [GrossAmount], 
       CAST(replace(tbl_GMA_Export_Promotions.[NetSalesAmountAfterDiscount], ',', '.') AS FLOAT) [NetSalesAmountAfterDiscount], 
       tbl_GMA_Export_Promotions.[UnitsSold], 
       CAST(replace(tbl_GMA_Export_Promotions.[AvgFPCostAvg], ',', '.') AS FLOAT) [AvgFPCostAvg], 
       CAST(replace(tbl_GMA_Export_Promotions.[NetSalesAmount], ',', '.') AS FLOAT) [NetSalesAmount], 
       CAST(replace(tbl_GMA_Export_Promotions.[FPCost], ',', '.') AS FLOAT) [FPCost], 
...
(
    ...
    GROUP BY BusinessDay, 
             ...
             PromotionName
                                         ...
                                         AND tbl_GMA_Export_Promotions.BusinessDay = tbl_GMA_Export_Promotions_newest.BusinessDay
